...
class Redis
{
    ...
    {
        Redis::$config = array_merge(self::$config, config('redis'));
        if (is_null(Redis::$handler)) {
            $class = '\\think\\redis\\driver\\Redis';  //此处部署Redis驱动所在位置，本例为org/redis/driver/Redis，实际根据位置修改
            Redis::$handler = new $class(Redis::$config);
        }
    }
    ...
    {
        Redis::init();
        return call_user_func_array([Redis::$handler, $method], $args);
    }
}